...
export async function createPTORequest(userId: string, input: { date: string; hours: number; reason: string }): Promise<IPTORequest> {
  ...
  if (input.hours < 1 || input.hours > 8) {
    throw new Error('PTO hours must be between 1 and 8');
  }
  ...
  if (hoursUsedThisMonth + input.hours > MONTHLY_PTO_HOURS) {
    ...
    throw new Error(`You have ${MONTHLY_PTO_HOURS - hoursUsedThisMonth} PTO hours remaining for ${monthName}`);
  }
...
export async function getUserPTORequests(userId: string, search?: string) {
  ...
  const requests = (await PTORequest.find(query).populate('userId', 'firstName lastName email').sort({ createdAt: -1 })) as IPTORequest[];
...
export async function updatePTORequestStatus(requestId: string, approverId: string, status: 'approved' | 'denied') {
  if (!['approved', 'denied'].includes(status)) {
    throw new Error('Invalid status');
  }

  const ptoRequest = (await PTORequest.findById(requestId).populate('userId', 'firstName lastName email')) as IPTORequest | null;
  if (!ptoRequest) {
...
/**
 * Utilities (ported from controller)
 */
