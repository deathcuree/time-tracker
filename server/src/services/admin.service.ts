...
export async function listAllUsers(): Promise<IUser[]> {
  return User.find().select('-password') as unknown as IUser[];
}

export async function userTimeEntries(params: { userId: string; startDate?: string; endDate?: string }): Promise<ITimeEntry[]> {
  const query: { userId: Types.ObjectId; date?: { $gte: Date; $lte: Date } } = {
  ...
  return TimeEntry.find(query).sort({ date: -1 }) as unknown as ITimeEntry[];
}
...
export async function updateRole(userId: string, role: 'user' | 'admin'): Promise<IUser | null> {
  ...
  return (await User.findByIdAndUpdate(userId, { role }, { new: true }).select('-password')) as IUser | null;
}

export async function exportPTORequestsTable(args: { search?: string; status?: 'pending' | 'approved' | 'denied' | 'all' }) {
  const pipeline: any[] = [
...
export async function listTimeLogs(args: {
  ...
  return { items, pagination: { total, page: pageNum, pages } };
}
